/**
 * Aplica a un buque un lote de planes de mantenimiento sacado de su plan en papel.
 *
 ...
 *
 * El buque sale del campo `vessel` del lote (por defecto LTE, que fue el primero
 * que se cargo con este script).
 *
 * Uso (en el VPS):
 *   export $(grep -E '^DATABASE_URL=' .env | xargs)
 *   DRY=1 npx tsx scripts/load-vessel-plan.ts scripts/_tmp-lte-cajas-plans.json
 *   npx tsx scripts/load-vessel-plan.ts scripts/_tmp-dch-mm-pp-bb-plans.json
 */
...
interface Lote {
  titulo: string;
  /** Codigo del buque; el usuario que firma la carga sale de USUARIO_POR_BUQUE. */
  vessel?: string;
  planes: PlanSpec[];
  assetFixes?: AssetFix[];
...
/** Quien figura como autor de la carga en cada buque: el jefe de maquinas. */
...
async function main() {
  ...
  const VESSEL = lote.vessel ?? "LTE";
  const USER_LEGACY_ID = USUARIO_POR_BUQUE[VESSEL];
  if (!USER_LEGACY_ID) throw new Error(`No hay usuario configurado para el buque ${VESSEL}`);
