// Guarda la foto/PDF de una OT llenada a mano en papel, subida para que la IA
// la lea y complete el formulario digital. Mismo patrón que
// fluid-uploads-service.ts (fluid-reports), carpeta propia.
...
const UPLOADS_ROOT = join(process.cwd(), "uploads", "wo-scans");
...
export function mimeForWorkOrderScanFile(filename: string): string {
  const ext = extname(filename).toLowerCase();
  return MIME_MAP[ext] ?? "application/octet-stream";
}

export async function saveWorkOrderScanFile(
  tenantSlug: string,
  originalName: string,
  buffer: Buffer,
  ...
  return {
    url: `/uploads/wo-scans/${tenantSlug}/${savedName}`,
    name: basename(originalName),
    mime: mimeForWorkOrderScanFile(originalName),
    absolutePath: filePath,
  };
}

export function readWorkOrderScanFile(tenantSlug: string, savedName: string): Buffer | null {
...
export function serveWorkOrderScanUpload(
  ...
  try {
    ...
    response.writeHead(200, {
      "Content-Type": mimeForWorkOrderScanFile(filename),
