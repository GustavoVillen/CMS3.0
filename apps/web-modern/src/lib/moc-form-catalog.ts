// Catálogo de preguntas del formulario controlado de Gestión de Cambios
// (REGI-GES-06.1). El texto se mantiene en español porque es el redactado
// oficial del documento controlado; no se traduce para no alterar su sentido.
//
// Las respuestas se guardan en MocRecord como JSON referenciando `n` (número):
//   evaluationAnswersJson:  [{ n, answer: "YES"|"NO"|"UNKNOWN", canImpact: boolean }]
//   effectivenessAnswersJson: [{ n, answer: "YES"|"NO", detail: string }]

export interface EvalQuestion { n: number; group: EvalGroupKey; text: string; }
export type EvalGroupKey =
  | "GENERAL" | "INFRASTRUCTURE" | "TECHNOLOGY" | "ORGANIZATIONAL" | "PROCEDURES";
...
// Sección 2 · Evaluación previa al cambio (24 preguntas Sí/No/No sabe + ¿impacta?)
...
// Sección 6 · Análisis de eficacia (7 preguntas Sí/No + Detalles/Fundamento)
export interface EffQuestion { n: number; text: string; }
export const EFFECTIVENESS_QUESTIONS: EffQuestion[] = [
...
// Áreas / personas que deberán evaluar el cambio (equipo multidisciplinario).
...
// Dimensión del cambio · Tipo (múltiple) — sección 1.
...
// Dimensión del cambio · Lugar donde se realizará (tipo).
