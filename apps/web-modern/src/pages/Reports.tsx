...
const InventoryTab: React.FC<{ vesselCode: string }> = ({ vesselCode }) => {
  ...
  if (!vesselCode) {
    return <p className="text-xs text-white/40 px-4 py-8 text-center">Seleccioná un buque para generar el reporte de inventario.</p>;
  }

  return (
    <div className="space-y-4">
      <div className="flex items-end gap-3 flex-wrap bg-white/5 border border-white/10 rounded-lg p-3">
      ...
      {loading && <p className="text-xs text-white/40 px-4 py-8 text-center"><Loader2 className="w-4 h-4 animate-spin inline mr-2" />Cargando inventario…</p>}
        ...
        <>
          <div className="grid grid-cols-3 gap-3 text-xs">
            <div className="bg-white/5 border border-white/10 rounded-lg p-3">
              ...
              <div className="text-white font-semibold">{data.vessel.code} — {data.vessel.name}</div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-lg p-3">
              ...
              <div className="text-white text-xl font-bold">{data.summary.totalItems}</div>
            </div>
            <div className={`border rounded-lg p-3 ${data.summary.belowReorderCount > 0 ? "bg-red-500/10 border-red-500/30" : "bg-white/5 border-white/10"}`}>
              <div className="text-[10px] uppercase tracking-wider text-white/40 mb-1">Bajo reorder</div>
              <div className={`text-xl font-bold ${data.summary.belowReorderCount > 0 ? "text-red-400" : "text-white"}`}>{data.summary.belowReorderCount}</div>
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-lg overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="text-white/80">
                {data.items.length === 0 && (
                  <tr><td colSpan={8} className="px-3 py-6 text-center text-white/30">Sin ítems para los filtros seleccionados.</td></tr>
                )}
                {data.items.map(it => (
                  <tr key={it.id} className={`border-b border-white/5 hover:bg-white/5 ${it.belowReorder ? "text-red-400" : ""}`}>
...
const ConsumptionTab: React.FC<{ vesselCode: string }> = ({ vesselCode }) => {
  ...
  if (!vesselCode) {
    return <p className="text-xs text-white/40 px-4 py-8 text-center">Seleccioná un buque para generar el reporte de consumo mensual.</p>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && <p className="text-xs text-white/40 px-4 py-8 text-center"><Loader2 className="w-4 h-4 animate-spin inline mr-2" />Cargando consumo…</p>}

      {data && (
        <>
          <div className="grid grid-cols-3 gap-3 text-xs">
            <div className="bg-white/5 border border-white/10 rounded-lg p-3">
              <div className="text-[10px] uppercase tracking-wider text-white/40 mb-1">Período</div>
              <div className="text-white font-semibold">{MONTH_NAMES_ES[data.period.month - 1]} {data.period.year}</div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-lg p-3">
              <div className="text-[10px] uppercase tracking-wider text-white/40 mb-1">Ítems consumidos</div>
              <div className="text-white text-xl font-bold">{data.summary.totalLines}</div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-lg p-3">
              <div className="text-[10px] uppercase tracking-wider text-white/40 mb-1">Cantidad total</div>
              <div className="text-white text-xl font-bold">{data.summary.totalConsumedQty.toFixed(2)}</div>
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-lg overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="text-white/80">
                {data.items.length === 0 && (
                  <tr><td colSpan={7} className="px-3 py-6 text-center text-white/30">Sin consumo registrado para el período.</td></tr>
                )}
                {data.items.map(it => (
                  <tr key={it.spareId} className="border-b border-white/5 hover:bg-white/5">
                    <td className="px-3 py-2 font-mono">{it.sku}</td>
                    <td className="px-3 py-2">{it.name}</td>
                    <td className="px-3 py-2 font-mono text-[10px]">{it.sfiCode ?? "—"}</td>
                    <td className="px-3 py-2 text-[10px]">{it.department ?? "—"}</td>
...
export const ReportsPage: React.FC = () => {
  ...
  return (
    <div className="px-6 py-4">
      ...
      {!selectedVesselCode && (
        <div className="mt-3 mb-4 flex items-end gap-3 flex-wrap bg-white/5 border border-white/10 rounded-lg p-3">
      ...
      <div className="mt-4 flex gap-2 border-b border-white/10">
        <button
          ...
          className={`px-4 py-2 text-xs font-semibold transition-colors border-b-2 -mb-px ${tab === "inventory" ? "border-accent text-accent" : "border-transparent text-white/40 hover:text-white/70"}`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 text-xs font-semibold transition-colors border-b-2 -mb-px ${tab === "consumption" ? "border-accent text-accent" : "border-transparent text-white/40 hover:text-white/70"}`}
        >
